...
type Stats = {
  ...
  meilleurParraineur: { nom: string; totalPoints: number; nombreFilleulsDirects: number } | null
}
...
export default function AdminReferralsPage() {
  ...
  const { data, isLoading } = useQuery<ApiResponse>({
    queryKey: ["admin-referrals", search, page],
    queryFn:  () =>
      fetch(`/api/admin/referrals?search=${encodeURIComponent(search)}&page=${page}&limit=20`)
        .then(r => r.json()),
  })
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-white">Parrainage — Vue complète</h1>
        <p className="text-sm text-[#666] mt-1">Suivi des points et chaînes de parrainage de tous les patients</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-[#2A2A2A] bg-[#141414] p-5">
          <div className="flex items-center gap-3 mb-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-full bg-[rgba(234,179,8,0.1)]">
              <Trophy className="h-5 w-5 text-yellow-400" />
            </div>
            <span className="text-sm text-[#888]">Meilleur parraineur</span>
          </div>
          {isLoading ? (
            <p className="text-xl font-bold text-white">—</p>
          ) : stats?.meilleurParraineur ? (
            <div>
              <p className="text-base font-bold text-white leading-tight">{stats.meilleurParraineur.nom}</p>
              <p className="text-sm text-[#888] mt-0.5">
                {stats.meilleurParraineur.totalPoints.toFixed(2)} pts · {stats.meilleurParraineur.nombreFilleulsDirects} filleuls
              </p>
            </div>
          ) : (
            <p className="text-[#666] text-sm">Aucun parraineur</p>
          )}
        </div>
      </div>
                  ...
                  <tr key={p.id} className="hover:bg-[#141414] transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      <button
                        ...
                        className="flex items-center gap-1.5 rounded-lg border border-[#2A2A2A] px-3 py-1.5 text-xs font-medium text-[#888] hover:border-[#C8906A] hover:text-[#C8906A] transition-colors"
                      >
